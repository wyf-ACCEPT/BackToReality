...
class PointnetSAModuleVotesRlt(nn.Module):
    ''' Modified based on _PointnetSAModuleBase and PointnetSAModuleMSG
    with extra support for returning point indices for getting their GT votes '''
    ...
    def __init__(
        ...
        if npoint is not None:
            self.grouper = pointnet2_utils.QueryAndGroupRlt(radius, nsample,
                use_xyz=use_xyz, ret_grouped_xyz=True, normalize_xyz=normalize_xyz,
                sample_uniformly=sample_uniformly, ret_unique_cnt=ret_unique_cnt)
        else:
            self.grouper = pointnet2_utils.GroupAll(use_xyz, ret_grouped_xyz=True)
...
class PointnetSAModuleOffset(nn.Module):
    ''' Modified based on _PointnetSAModuleBase and PointnetSAModuleMSG
    with extra support for returning point indices for getting their GT votes '''
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        features : torch.Tensor
            (B, C, N) tensor of the descriptors of the the features
        new_xyz : torch.Tensor
            (B, npoint, 3) tensor of the xyz coordinates of the gt points
...
def ThreeNNInterpolate(known_feats, known_xyz, unknown_xyz):
